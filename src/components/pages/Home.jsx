...
import { getBicycles, deleteBicycle, deleteImage } from '../../services/services.js';
      ...
      <div className='gallery'>
        {bicycles.map((bicycle) => (
          <div className='gallerygrid' key={bicycle.id}>
            <img
              onClick={() => navigate(`/card/${bicycle.id}`)}
              className="bicyclesimg"
              src={bicycle.image}
              alt={bicycle.model}
            />
            <p>{bicycle.model}</p>
            <div className="gallery-button">
              ...
              <button
                ...
                onClick={() => {
                  const confirmDelete = window.confirm('¿Deseas eliminar esta bicicleta?');
                  if (confirmDelete) {
                    handleDelete(bicycle.id, bicycle.image);
                  }
                }}
              >
                Eliminar
              </button>
              {/* Agrega aquí el botón de "me gusta" si lo deseas */}
            </div>
          </div>
        ))}
      </div>
